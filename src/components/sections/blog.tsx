...
const Blog = () => {
  return (
    <section id="blog" className="py-20 md:py-32 bg-light-ivory">
      <div className="container mx-auto px-4">
        ...
        <div className="grid md:grid-cols-3 gap-8">
          {blogPosts.map((post) => (
            <div key={post.slug} className="bg-white rounded-lg shadow-md overflow-hidden transform hover:-translate-y-2 transition-transform duration-300">
              <Image
                src={post.image}
                alt={post.title}
                width={600}
              ...
              />
              <div className="p-6">
                <p className="text-sm text-slate-gray mb-2">{post.date}</p>
                <h3 className="font-bold text-xl text-deep-navy mb-3">{post.title}</h3>
                <p className="text-slate-gray mb-4">{post.summary}</p>
